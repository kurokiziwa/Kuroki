    ...
    try {
        ...
        if (!teks) {
            ...
            let text = menuList.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), 
                (_, name) => '' + replace[name])
            await conn.sendMessage(m.chat, { image: { url: 'https://raw.githubusercontent.com/kurokiziwa/Website/refs/heads/main/Screenshot%202026-05-12%20024942.png' }, caption: text, mentions: [m.sender] }, { quoted: m });
            return
        }
